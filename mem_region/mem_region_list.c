 ...
 */
bool mem_region_list_init(mem_region_list_t *list, mem_addr_t address, mem_size_t length, struct mem_region *entries,
                          int32_t entry_count) {
 ...
 */
mem_size_t mem_region_list_get_used(const mem_region_list_t *list) { return (list != NULL) ? list->used.length : 0u; }
/**
 * Getting free size.
 *
 ...
 */
mem_size_t mem_region_list_get_free(const mem_region_list_t *list) { return (list != NULL) ? list->free.length : 0u; }
